# [E 1-1] 내 컴퓨터에 개발자용 '작업실' 꾸미기

## 프로젝트 개요

개발 워크스테이션 환경을 구성해보자. 핵심 도구인 리눅스 CLI(터미널), Docker(컨테이너), Git/GitHub(버전 관리 및 협업)을 다루어 보게 될 것이다.

  * 터미널로 작업 디렉토리와 권한을 정리, Docker를 설치 및 점검하고 컨테이너를 실행/관리

  * 간단한 웹 서버를 Dockerfile로 컨테이너화, 포트 매핑으로 접속을 확인, 바인드 마운트/볼륨으로 "변경 반영"과 "데이터 영속성"을 직접 검증

  * 실행 결과(로그/접속/데이터 유지)로 핵심 흐름을 확인
...
### 7. 포트 매핑 및 접속 증거
 * __도커 포트 매핑이 필요한 이유__
 
__네트워크 격리 구조 해결 :__

  도커 컨테이너는 기본적으로 독립된 가상 네트워크 환경에서 실행됩니다. 

  컨테이너 안에서 실행 중인 웹 서버 등의 서비스는 외부에서 직접 접근할 수 없습니다.

__외부 접속 허용 (포트 포워딩):__

  호스트(실제 컴퓨터)의 포트와 컨테이너 내부의 포트를 연결(-p 옵션)합니다.

  호스트의 특정 포트로 들어온 요청을 컨테이너 내부의 포트로 전달해 줍니다.
```bash
...
### 11. 추가 내용
 * __호스트 포트가 이미 사용 중이라 매핑이 불가능한 경우?__
1. __포트를 사용하는 프로세스 찾기__
   
   Linux / macOS: 터미널에 lsof -i :포트번호 또는 ss -tunlp | grep 포트번호를 입력
   
   Windows: 명령 프롬프트(CMD)에 netstat -ano | findstr 포트번호를 입력
   
   출력 결과에서 PID(프로세스 고유 번호)를 확인
2. __프로세스 정체 파악하기__
   
   다른 도커 컨테이너인지 확인: docker ps 명령어로 실행 중인 도커 목록을 봅니
   
   시스템 서비스인지 확인: 윈도우 작업 관리자나 리눅스 ps -ef | grep PID 명령어로 일반 프로그램인지 확인
   
3. __도커 설정 및 중복 실행 점검__
   
   docker ps -a 명령어로 꺼진 줄 알았던 이전 컨테이너가 백그라운드에서 포트를 잡고 있는지 확인
   
   docker-compose.yml 파일이나 실행 명령어(docker run -p)에 적힌 호스트 포트 번호가 중복되었는지 확인
---
이하 미션 원문
---
1. 미션 소개
